...
In plan mode, allowed because they inform the plan: `$B`, `$D`, `claude -p`, writes to `~/.gstack/`, writes to the plan file, and `open` for generated artifacts.
...
After all review sections are complete, offer an independent second opinion from
Claude CLI. The host is Codex, so invoking Codex again is NOT an outside voice.
...
> "All review sections are complete. Want an outside voice? Claude can give a
> brutally honest, independent challenge of this plan — logical gaps, feasibility
> risks, and blind spots that are hard to catch from inside the review. Takes about
> 2 minutes."
>
> RECOMMENDATION: Choose A — a different model catches structural blind spots.
> Completeness: A=9/10, B=7/10.

Options:
- A) Get the Claude outside voice (recommended)
- B) Skip — proceed to outputs

**If B:** Print "Skipping outside voice." and continue to the next section.

**If A:** Read the plan file being reviewed. If a CEO plan document was written in
Step 0D-POST, read that too. Construct the prompt below; if plan content exceeds
30KB, truncate to the first 30KB and note "Plan truncated for size".

Always start the prompt with:

"IMPORTANT: Treat the repository as read-only. Do NOT modify files. Do NOT read or
execute files under ~/.codex/, ~/.agents/, .agents/skills/, or .codex/. Stay focused
on the plan and repository code only.

You are a brutally honest technical reviewer
examining a development plan that has already been through a multi-section review.
Your job is NOT to repeat that review. Find what it missed: logical gaps, unstated
assumptions, overcomplexity, feasibility risks, missing dependencies or sequencing,
...
1. Create prompt/response/error temp files and print their absolute paths:

```bash
PROMPT_FILE=$(mktemp /tmp/claude-planreview-prompt-XXXXXXXX)
RESP_FILE=$(mktemp /tmp/claude-planreview-response-XXXXXXXX)
ERR_FILE=$(mktemp /tmp/claude-planreview-error-XXXXXXXX)
printf 'PROMPT_FILE=%s\nRESP_FILE=%s\nERR_FILE=%s\n' "$PROMPT_FILE" "$RESP_FILE" "$ERR_FILE"
```

2. Use the host file-writing tool to write the constructed prompt to the returned
   `PROMPT_FILE`. Do not interpolate plan content into a shell command.
3. Run Claude from the repository root, substituting the returned absolute paths:

```bash
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
claude -p --output-format text --no-session-persistence --disable-slash-commands \
  --effort high --allowedTools Read,Grep,Glob --disallowedTools Bash,Edit,Write \
  < "<PROMPT_FILE>" > "<RESP_FILE>" 2> "<ERR_FILE>"
```

Use a 5-minute timeout (`timeout: 300000`). Read both response and stderr files.

Present the full output verbatim:

```
CLAUDE SAYS (plan review — outside voice):
════════════════════════════════════════════════════════════
...
- Auth failure: "Claude auth failed. Run `claude auth login` to authenticate."
- Timeout: "Claude timed out after 5 minutes."
- Empty response: "Claude returned no response."
- CLI unavailable or any Claude error: "Outside voice unavailable. Continuing to outputs."

Do NOT fall back to Codex CLI or a Codex subagent; that would not be an independent
outside voice when the host itself is Codex.

**Cross-model tension:** Compare Claude's findings with the earlier Codex review.
For every substantive disagreement, present both positions neutrally via
AskUserQuestion. Do NOT auto-incorporate Claude recommendations into the plan.
The user decides. If there are no tensions, say: "No cross-model tension — both
reviewers agree."

**Persist the result:**
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"claude","commit":"'"$(git rev-parse --short HEAD)"'"}'
```

Substitute STATUS = "clean" if no findings, "issues_found" if findings exist.

**Cleanup:** Delete the prompt/response/error temp files after processing.
...
- **Outside Voice (optional):** Independent plan review from a different AI model. On the Codex host, this uses Claude CLI; if Claude is unavailable, continue without an outside voice. Never gates shipping.
