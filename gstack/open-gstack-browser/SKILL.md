...
[ -z "$B" ] && B="$HOME/.agents/skills/gstack/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`, run setup yourself. Do not ask the user to click confirm for
this one-time build.
...
If `bun` is not installed, install it with checksum verification, then rerun
setup:
...
After setup, rerun the readiness check. Stop only if setup fails or the `browse`
...
## Step 3: Agent-side visibility check

Do not use AskUserQuestion or stop for the user to confirm they can see the
browser or Side Panel. If `$B status` shows `Mode: headed` and `$B snapshot -i`
returns page content, the agent can see enough to proceed.
...
If the output shows an error, the mode is not `headed`, or the snapshot cannot
read the page:
1. Run `$B status` and show the output.
2. If the server is not healthy, re-run Step 0 cleanup + Step 1 connect.
3. If the server is healthy but the browser is not visible, try `$B focus`.
4. Ask the user what they see only if these agent-side checks still cannot
   identify the problem.

Side Panel setup is optional. Give manual Side Panel instructions only when the
user explicitly wants live viewing, pairing, or sidebar chat. Otherwise, proceed
with the requested QA, smoke, replay, screenshot, or browser task.

Manual Side Panel instructions, only when needed:
1. Click the puzzle piece icon in the Playwright-controlled Chrome toolbar.
2. Pin **gstack browse**.
3. Click the pinned gstack icon to open the Side Panel.
4. If the badge stays disconnected, enter port **34567** manually.

## Step 4: Optional manual-viewing demo

Skip this demo for agent-run verification tasks. Run it only when the user's
goal is onboarding, live watching, Side Panel setup, pairing, or manual browser
control.
...
Tell the user: "The browser is connected. The Side Panel activity feed should
show the `goto` and `snapshot` commands if you opened it."
...
## Step 7: Auto-close after agent-run verification

If this browser was opened so the agent could verify something itself — QA,
smoke testing, replay, screenshots, visual checks, or evidence collection —
keep the headed browser open only while those commands are still needed.

After the final evidence is captured and before reporting back, run:

```bash
$B disconnect
```

In the final report, include: `GStack Browser: closed after verification`.

Skip this auto-close only when:
- The user explicitly asks to keep the browser open
- The user's goal is manual browser/Side Panel setup, pairing, or live control
- A later, already-planned step still needs the same headed session

If you skip auto-close, say why in the final report. If `$B disconnect` fails,
run `$B status` and report the remaining browser state.
