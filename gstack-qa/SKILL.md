...
```bash
_QA_STASH_MARKER="$(git rev-parse --git-path gstack-qa-auto-stash)"
rm -f "$_QA_STASH_MARKER"
_DIRTY=$(git status --porcelain)
if [ -n "$_DIRTY" ]; then
  _QA_STASH_TS=$(date +%Y%m%d-%H%M%S)
  if git stash push -u -m "pre-qa-auto-${_QA_STASH_TS}"; then
    printf '%s\n' "pre-qa-auto-${_QA_STASH_TS}" > "$_QA_STASH_MARKER"
    echo "QA_AUTO_STASHED: pre-qa-auto-${_QA_STASH_TS}"
  else
    echo "QA_STASH_FAILED"
    exit 1
  fi
else
  echo "QA_WORKTREE_CLEAN"
fi
```

If a stash was created, continue QA against the clean tree. Do not ask the user to choose commit/stash/abort, and do not commit their unrelated work. At the end of QA, run:

```bash
_QA_STASH_MARKER="$(git rev-parse --git-path gstack-qa-auto-stash)"
if [ -f "$_QA_STASH_MARKER" ]; then
  if git stash pop; then
    rm -f "$_QA_STASH_MARKER"
  else
    echo "QA_STASH_POP_CONFLICT"
    git status --short
    exit 1
  fi
fi
...
If `git stash push` fails, stop and report the blocker. If `git stash pop` creates conflicts, stop immediately, report the conflicted files, and leave conflict resolution to the user.
...
If `NEEDS_SETUP`:
1. Run `cd <SKILL_DIR> && ./setup` yourself. This is a one-time local build; do not ask the user for confirmation.
2. If setup fails, report the error and stop.
...
11. **Clean working tree required.** If dirty, automatically run `git stash push -u -m "pre-qa-auto-${_QA_STASH_TS}"` before proceeding. Do not ask the user to choose commit/stash/abort. Pop the stash at the end and stop on conflicts.
