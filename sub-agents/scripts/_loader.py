...
def validate_timeout(value: str | None) -> int | None:
    """Validate an optional positive per-agent idle timeout in milliseconds."""
    if value is None or value == "":
        return None
    try:
        timeout_ms = int(value)
    except ValueError as exc:
        raise ValueError(f"Invalid timeout {value!r}: expected milliseconds.") from exc
    if timeout_ms <= 0:
        raise ValueError(f"Invalid timeout {value!r}: expected a positive integer.")
    return timeout_ms


def load_agent(
    agents_dir: str, agent_name: str
) -> tuple[str | None, str, str, str, str, str | None, str | None, int | None]:
    ...
    for ext in [".md", ".txt"]:
        ...
        if resolved.exists():
            ...
            timeout_ms = validate_timeout(frontmatter.get("timeout"))
