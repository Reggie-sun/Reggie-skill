...
This section is a /learn-specific override for Codex-generated skills. It only
changes how this learning manager interacts with the user; it does not change
gstack's global AskUserQuestion rules for other skills.

- Read-only commands (`/learn`, `/learn search`, `/learn stats`) must proceed without `AskUserQuestion`.
- Decision commands (`/learn prune`, `/learn export`, `/learn add`) should use the host's available user-input tool when `AskUserQuestion` is unavailable. If no structured prompt tool is available, present the exact options in chat and stop for the user's reply.
- Codex output resolves gstack binaries through `$GSTACK_ROOT` / `$GSTACK_BIN`. Run the preamble first, then use those variables instead of hardcoded Claude paths.
- Do not create or update `CLAUDE.md` just because the host is Codex. For exported learnings, prefer a separate markdown file, or `AGENTS.md` only when the project already uses it for durable agent guidance.
