...
description: |
  ...
  CSV-driven parallel coordination with NDJSON discovery board. Long-running role analysis is
  expected; first wait timeout means "keep polling", not "abort".
argument-hint: "[-y|--yes] [--count N] [--session ID] [--skip-questions] [--style-skill PKG] \"topic\" | <role-name> [--session ID]"
allowed-tools: spawn_agents_on_csv, spawn_agent, wait_agent, send_message, followup_task, list_agents, assign_task, close_agent, request_user_input, Read, Write, Edit, Bash, Glob, Grep
---
...
- This skill must be patient with slow role agents and synthesis agents.
- Do not treat the first `wait_agent` timeout as a failure.
- Prefer repeated polling windows of `120000` ms or longer for spawned agents.
- Only stop waiting early when there is an explicit failure, a hard runtime limit is reached, or the user cancels.
- Do not close and respawn a role agent just because it is slow; that causes duplicate work and can look like interruption.

## Subagent Continuity Rules

- Once a role agent or synthesis agent is spawned, it owns that subtask until it explicitly reports `completed` or `failed`.
- A silent or slow subagent is still an active subagent. Silence is not evidence of failure.
- Do not `close_agent`, respawn a duplicate, or overwrite its responsibility just because a poll window timed out.
- If you need to steer a slow subagent, use `send_message` to narrow scope; keep the same agent alive.
- If the parent turn is resumed after delay or compaction, first call `list_agents`, reuse any live brainstorm agents, and continue waiting instead of spawning replacements.
- Only use `close_agent` after completion has been observed, or after an explicit blocker/failure and the result has been recorded.
- If an external interruption happens, the recovery rule is: reconnect to the existing session state, enumerate live agents, and continue from there. Do not silently restart the wave.

### Shared Wait Helper

```javascript
function waitForAgentCompletion(agentId, label) {
  let timeoutCount = 0
  while (true) {
    const update = wait_agent({ targets: [agentId], timeout_ms: 120000 })
    if (!update?.timed_out) break

    timeoutCount += 1
    console.log(`[wait] ${label} still running after ${timeoutCount * 2} min; continuing to wait`)

    if (timeoutCount >= 10) {
      console.log(`[warn] ${label} is unusually slow, but the skill must preserve continuity and keep waiting unless there is an explicit blocker or cancellation.`)
    }
  }
}
```
...
```javascript
function resumeExistingBrainstormAgents(sessionFolder) {
  const liveAgents = list_agents({})
  const relevantAgents = (liveAgents?.agents || []).filter(agent =>
    agent.agent_name?.includes('brainstorm') ||
    agent.last_task_message?.includes(sessionFolder)
  )

  if (relevantAgents.length > 0) {
    console.log(`[resume] reusing ${relevantAgents.length} live brainstorm agent(s) for ${sessionFolder}`)
  }
...
1. **Role Analysis Wave**
   ...
   // Important: spawn_agents_on_csv can legitimately take a long time on larger topics.
   // Do not interpret a slow run as interruption unless the runner explicitly fails
   // or the per-worker runtime limit is exceeded. If the parent flow resumes mid-wave,
   // reattach to the existing session/agents instead of starting a second wave.
...
if (executionMode === 'single-role') {
  ...
  resumeExistingBrainstormAgents(sessionFolder)
...
`
  ...
  waitForAgentCompletion(agentId, `${roleName} role analysis`)
...
`
   ...
   resumeExistingBrainstormAgents(sessionFolder)
   waitForAgentCompletion(synthesisAgent, 'brainstorm synthesis')
...
| First `wait_agent` timeout | Keep polling; do not abort or respawn the agent |
| Slow synthesis / role analysis | Use repeated `120000` ms waits and only escalate on explicit failure or hard runtime cap |
| Parent turn resumes mid-execution | Reuse existing live agents via `list_agents`; do not start duplicate role waves |
...
> - After re-reading state, enumerate live agents and continue waiting on existing role/synthesis agents before considering any respawn
